...
**Q: Do I have to use polling?**  
A: No. Once you have the table and service layer, upgrading to WebSocket is a small change (2-3 hours). Start with polling, upgrade when you see latency complaints.

**Q: What if I'm using Prisma/TypeORM/SQLAlchemy?**  
A: The notification service becomes single method calls. No difference—same indices, same isolation logic.

**Q: Can I use NoSQL?**  
A: Not ideal. Notifications are strongly structured and query-heavy. Stick with SQL for this. NoSQL shines for flexible schemas (user profiles, metadata) not transactional lists.

**Q: How do I avoid duplicate notifications?**  
A: Idempotency key (like Stripe) in create endpoint:
```javascript
app.post('/notifications', authenticate, async (req, res) => {
  const { idempotencyKey, type, title, body } = req.body;
  ...
  const notif = await db.notifications.create({
    idempotencyKey, type, title, body, tenantId: req.tenantId
  });
...
A: Add an `channels` enum (in_app, email, sms) to the notification. When marking as read in-app, you've still sent the email. Email is separate from this system—it's a second delivery channel.

---

**Bottom line:** This fits cleanly into any existing product. You're adding one table, one service, five endpoints, and a React component. No architectural upheaval. You're ready to scale when it matters.
